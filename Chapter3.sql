...
3-3
  ...
  4.UPDATE 成績表 SET 外国語 = 81
     -- WHERE 学籍番号 = 'A002' AND 'E003'   -- 自分の回答
     WHERE 学籍番号 IN ('A002', 'E003')   -- 解答

  5. -- ルールは(1)から順に適応されるため、(2)以降の記述には 総合成績 IS NULLの記述が必要になる
    (1)UPDATE 成績表 SET 総合成績 = 'A'
        -- WHERE (法学 AND 経済学 AND 哲学 AND 情報理論 AND 外国語) > 80   -- 自分の回答
       WHERE 法学 >= 80 AND 経済学 >= 80 AND 哲学 >= 80 AND 情報理論 >= 80 AND 外国語 >= 80  -- 解答：80以上なので論理演算子には=が必要
    (2)UPDATE 成績表 SET 総合成績 = 'B'
        -- WHERE (法学 OR 外国語 > 80 ) AND (経済学 OR 哲学 >80)   -- 自分の回答
       WHERE (法学 >= 80 OR 外国語 >= 80 )
         AND (経済学 >= 80 OR 哲学 >= 80)
         AND 総合成績　IS  NULL   -- 解答：(1)の除外が必要なため
    (3)UPDATE 成績表 SET 総合成績 = 'C'
        -- WHERE (法学 AND 経済学 AND 哲学 AND 情報理論 AND 外国語) < 50   -- 自分の回答
       WHERE 法学 < 50 AND 経済学 < 50 AND 哲学 < 50 AND 情報理論 < 50 AND 外国語 < 50
         AND 総合成績 IS NULL
    (4)分からない
      -- (1)〜(3)に当てはまらないものは総合成績がNULLになっている
...
3-4
  1.ない		-- 解答：月(NULLではなく、月が被ることはないため)
  2.コード
  3.学籍番号